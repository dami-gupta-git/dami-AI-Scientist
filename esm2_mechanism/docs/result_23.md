# Result 23 — Pathogenicity is encoded in the *direction* of the ESM-2 delta, not its magnitude

## Date: 2026-05-28 | Script: magnitude_direction.py | Seeds: 0–4 | CPU only

> **STATUS: confirmed across 5 seeds** (std ≤ 0.002). The full-delta baseline reproduces result_6 exactly (family-split MLP AUROC 0.884). Probe C (S1724 signed ΔΔG) ran but needs re-running with a median binarization before it can be cited — see caveat below. Numbering reserves result_22 for the log-likelihood scan (plan_loglik.md).

---

## TL;DR

result_6 showed the same ESM-2 delta predicts pathogenicity (AUROC 0.886) but not mechanism. Decomposing the delta `d = mut_emb − wt_emb` into **magnitude** `‖d‖` and **direction** `d/‖d‖` shows the pre-registered hypothesis (pathogenicity = magnitude) is **falsified in the opposite direction**: pathogenicity is carried by the **direction** (family-split AUROC 0.896 ≈ full delta), while magnitude-only is weak (0.664). Geometry follow-up: that direction is a single, low-dimensional, redundantly-encoded functional axis. **Unified finding (transfer contrast across tasks × probe type, consistent with result_21):** pathogenicity is a **linear family-universal axis** (linear transfer AUROC 0.815, GBM 0.889); stability is a **nonlinear cross-family manifold** (linear transfer 0.725 but GBM recovers it — 0.761 here, 0.750 on result_21's clean Pfam-split); mechanism has **no transferable signal at any probe level** (transfer ≈ chance for both linear 0.520 and GBM 0.540 — nonlinearity does not rescue it). Transferability is **task-dependent and probe-dependent within one frozen model** — a positive, interpretable geometric claim, not a bare null.
...
**Net:** pathogenicity is a single, low-dimensional, family-transferable functional direction — universal in *effect*, redundantly encoded rather than a clean unique vector.
...
- `scripts/transfer_contrast.py` — same transfer test across pathogenicity / stability / mechanism (the decider)
- `results/magnitude_direction/probe_results.json` — 5-seed results + gate outcomes
- `results/magnitude_direction/geometry_results.json` — rank + family-transfer results
- `docs/plans/plan_magnitude_direction.md` — pre-registration + progress log
